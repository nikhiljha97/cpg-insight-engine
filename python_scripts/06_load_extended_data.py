...
EXCEL_PATH = "dunnhumby-Breakfast-at-the-Frat.xlsx"
...
if os.path.exists("dh_product_lookup.csv"):
    cp = pd.read_csv("dh_product_lookup.csv")
...
if os.path.exists("dh_causal_lookup.csv"):
    cc = pd.read_csv("dh_causal_lookup.csv")
...
if os.path.exists("dh_transactions.csv"):
    print("  Loading dh_transactions.csv (~5.2M rows, takes ~60s)...")
    ct = pd.read_csv("dh_transactions.csv")
...
# Try sample files first, then full parts
sample_files = sorted(glob.glob("*50k*customers*.csv") + glob.glob("*50K*customers*.csv") +
                      glob.glob("sample_50k*.csv") + glob.glob("5000-customers*.csv") +
                      glob.glob("50000-customers*.csv"))
part_files   = sorted(glob.glob("*part*.csv") + glob.glob("*Part*.csv") +
                      glob.glob("lets-get-sort-of-real-part*.csv"))
